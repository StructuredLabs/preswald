...
def test_millisecond_precision():
    ...
    try:
        ...
        try:
            ...
            pytest.fail(
                f"Timestamp {timestamp} does not match expected base format with milliseconds"
            )
...
def test_custom_timestamp_format():
    ...
    try:
        ...
        try:
            ...
            assert sep == "," and ms.isdigit() and len(ms) == 3, (
                f"Milliseconds part '{ms}' is not 3 digits after comma in '{timestamp}'"
            )
        except ValueError:
            assert False, (

...
text("## Filters")
weather_threshold = slider("Select Weather Type (0=All, 1=Sun, 2=Rain, 3=Drizzle, 4=Snow)", 
                          min_val=0, 
                          max_val=4, 
                          default=0)

month_threshold = slider("Select Month (0=All, 1=Jan, 2=Feb, 3=Mar, 4=Apr, 5=May, 6=Jun, 7=Jul, 8=Aug, 9=Sep, 10=Oct, 11=Nov, 12=Dec)", 
                        min_val=0, 
                        max_val=12, 
                        default=0)
...
fig_monthly.update_layout(
    ...
    yaxis=dict(title='Total Precipitation (mm)', side='left', showgrid=False),
    yaxis2=dict(title='Average Temperature (°C)', side='right', overlaying='y', showgrid=False),
    template='plotly_white',
    legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1),
    margin=dict(l=40, r=40, t=60, b=40)
)
...
precip_stats = pd.DataFrame({
    'Metric': ['Total Annual Precipitation (mm)', 

...
fig_scatter_bp = px.scatter(
    ...
    labels={'ap_hi': 'Systolic Blood Pressure', 'ap_lo': 'Diastolic Blood Pressure', 'cardio': 'Cardio Outcome'}
)
fig_scatter_bp.update_layout(
    xaxis=dict(title='Systolic Blood Pressure', range=[df['ap_hi'].min() - 10, df['ap_hi'].max() + 10]),
    yaxis=dict(title='Diastolic Blood Pressure', range=[df['ap_lo'].min() - 10, df['ap_lo'].max() + 10])
)
plotly(fig_scatter_bp)

# 3D Scatter plot for 'weight', 'age', and 'ap_hi' by 'cardio'
text("### 3D Scatter Plot: Weight, Age, and Systolic Blood Pressure by Cardio Outcome (Processed Data)")
...
selected_features = ['age', 'height', 'weight', 'ap_hi', 'ap_lo', 'cholesterol', 'cardio']
